...
class User extends CI_Controller
{
    ...
    public function profile() 
    {
		...
		'is_aktif'      => $row->is_aktif,
            ...
            $this->template->load('template','auth/profile', $data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
    ...
    public function ganti_pass($id) 
    {
        ...
        if ($row) {
            $data = array(
                'button'        => 'Update',
                'action'        => site_url('user/ganti_pass_action'),
		        'id_users'      => set_value('id_users', $row->id_users));
            $this->template->load('template','user/tbl_user_form_password', $data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('user'));
        }
    ...
    public function ganti_pass_action() 
    {
            $password       = $this->input->post('password',TRUE);
            $options        = array("cost"=>4);
            $hashPassword   = password_hash($password,PASSWORD_BCRYPT,$options);
    ...
    public function delete($id) 
    {
        ...
        $row1 = $this->User_model->get_by_id($this->session->userdata('id_users'));        
    ...
    public function refresh() {
        ...
        if($row1) {
            $this->session->set_flashdata('message', 'Akun Anda Telah Dihapus');
