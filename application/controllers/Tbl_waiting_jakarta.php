...
class Tbl_waiting_jakarta extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($q <> '') {
            ...
            $config['first_url'] = base_url() . 'index.php/tbl_waiting_jakarta/index.html?q=' . urlencode($q);
        } else {
            $config['base_url'] = base_url() . 'index.php/tbl_waiting_jakarta/index/';
            $config['first_url'] = base_url() . 'index.php/tbl_waiting_jakarta/index/';
        }

        $config['per_page'] = 10;
        $config['page_query_string'] = FALSE;
        $config['total_rows'] = $this->Tbl_waiting_jakarta_model->total_rows($q);
        $tbl_waiting_jakarta = $this->Tbl_waiting_jakarta_model->get_all();
        ...
        $data = array(
            'tbl_waiting_jakarta_data' => $tbl_waiting_jakarta,
            'q' => $q,
        ...
        $this->template->load('template','tbl_waiting_jakarta/tbl_waiting_jakarta_list', $data);
    }
    ...
    public function sampah()
    {
        ...
        $this->template->load('template','tbl_waiting_jakarta/tbl_waiting_jakarta_list_sampah', $data);
    }
    ...
    public function read($id) 
    {
        ...
        if ($row) {
            $data = array(
		'id_waiting_jakarta' => $row->id_waiting_jakarta,
		...
		'batal' => $row->batal,
            ...
            $this->template->load('template','tbl_waiting_jakarta/tbl_waiting_jakarta_read', $data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('tbl_waiting_jakarta'));
        }
    }
    ...
    public function create() 
    {
        $data = array(
            'button' => 'Create',
            'action' => site_url('tbl_waiting_jakarta/create_action'),
            'tbl_spk_data' => $this->Tbl_waiting_jakarta_model->get_all_spk(),
            'id_waiting_jakarta' => set_value('id_waiting_jakarta'),
        ...
        $this->template->load('template','tbl_waiting_jakarta/tbl_waiting_jakarta_form', $data);
    }
    ...
    public function create_action() 
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->Tbl_waiting_jakarta_model->insert($data);
    ...
    public function update($id) 
    {
        ...
        if ($row) {
            $data = array(
                'button' => 'Update',
                'action' => site_url('tbl_waiting_jakarta/update_action'),
                'tbl_spk_data' => $this->Tbl_waiting_jakarta_model->get_all_spk(),
    ...
    public function update_action() 
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            $this->update($this->input->post('id_waiting_jakarta', TRUE));
        } else {
            $data = array(
            'id_sub_jakarta' => $this->input->post('id_sub_jakarta',TRUE),
            'jumlah_permintaan' => $this->input->post('jumlah_permintaan',TRUE),
    ...
    public function restore_sampah($id) 
    {
            ...
            redirect(site_url('tbl_waiting_jakarta/sampah'));
    }
    ...
    public function delete_list_kasir($id) 
    {
        ...
        if ($row) {
            ...
            $this->Tbl_waiting_jakarta_model->delete($id);
            $this->session->set_flashdata('message', 'Delete Record Success');
            redirect(site_url('tbl_sub_jakarta/lihat_waiting/'.$idSub.''));
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('tbl_sub_jakarta/lihat_waiting/'.$idSub.''));
        }
	...
	$this->form_validation->set_rules('id_waiting_jakarta', 'id_waiting_jakarta', 'trim');
    ...
    public function word()
    {
        ...
        header("Content-Disposition: attachment;Filename=tbl_waiting_jakarta.doc");

        $data = array(
            'tbl_waiting_jakarta_data' => $this->Tbl_waiting_jakarta_model->get_all(),
            'start' => 0
        );
        
        $this->load->view('tbl_waiting_jakarta/tbl_waiting_jakarta_doc',$data);
    }
    ...
    public function cari()
    {
        ...
        $data = array(
            'tbl_waiting_jakarta_data' => $this->Tbl_waiting_jakarta_model->get_by_id_cari($acc),
            'start' => 0,
            'pagination' => $this->pagination->create_links(),
    ...
    public function cari_tanggal()
    {
        ...
        $data = array(
            'tbl_waiting_jakarta_data' => $this->Tbl_waiting_jakarta_model->get_by_id_cari_tanggal($bulan, $tahun),
            'start' => 0,
            'pagination' => $this->pagination->create_links(),
...
/* End of file Tbl_waiting_jakarta.php */
/* Location: ./application/controllers/Tbl_waiting_jakarta.php */
