...
class Tbl_spk_banjarmasin_model extends CI_Model
{

    public $table = 'tbl_spk_banjarmasin';
    public $id = 'id_spk_banjarmasin';
    public $order = 'DESC';
    ...
    function get_all()
    {
        ...
        $this->db->join('tbl_user' , 'tbl_user.id_users = tbl_spk_banjarmasin.id_users' , 'LEFT');
        $this->db->join('tbl_vendor' , 'tbl_vendor.id_vendor = tbl_spk_banjarmasin.id_vendor' , 'LEFT');
        $this->db->join('tbl_vic' , 'tbl_vic.id_vic = tbl_spk_banjarmasin.id_vic' , 'LEFT');
        if($this->uri->segment(2) == 'sampah') {
            $this->db->where('tbl_spk_banjarmasin.hapus', 1);
        } else {
    ...
    function get_by_vic($id_vic)
    {
        ...
        $this->db->join('tbl_vendor' , 'tbl_vendor.id_vendor = tbl_spk_banjarmasin.id_vendor' , 'LEFT');
        $this->db->join('tbl_vic' , 'tbl_vic.id_vic = tbl_spk_banjarmasin.id_vic' , 'LEFT');
        $this->db->where('tbl_spk_banjarmasin.id_vic', $id_vic);
        if($this->uri->segment(2) == 'sampah') {
            $this->db->where('tbl_spk_banjarmasin.hapus', 1);
        } else {
            $this->db->where('tbl_spk_banjarmasin.hapus', 0);
    ...
    function get_nilai_spk($id)
    {
        ...
        $this->db->where('id_spk_banjarmasin', $id);
        if($this->uri->segment(2) == 'sampah') {
        ...
        return $this->db->get('tbl_sub_banjarmasin')->row();
    }

    function get_total_nilai_spk()
    {
        $this->db->select('SUM(tbl_sub_banjarmasin.nilai_spk) as total');
        $this->db->join('tbl_spk_banjarmasin' , 'tbl_sub_banjarmasin.id_spk_banjarmasin = tbl_spk_banjarmasin.id_spk_banjarmasin' , 'LEFT');
        $this->db->join('tbl_vendor' , 'tbl_vendor.id_vendor = tbl_spk_banjarmasin.id_vendor' , 'LEFT');
        $this->db->join('tbl_vic' , 'tbl_vic.id_vic = tbl_spk_banjarmasin.id_vic' , 'LEFT');
    ...
    function total_rows($q = NULL) {
        $this->db->select('*');
        $this->db->like('id_spk_banjarmasin', $q);
...
/* End of file Tbl_spk_banjarmasin_model.php */
/* Location: ./application/models/Tbl_spk_banjarmasin_model.php */
