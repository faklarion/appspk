	...
	<section class="content">
		<div class="box box-warning box-solid">
			<div class="box-header with-border">
				<h3 class="box-title"><?php echo strtoupper($button) ?> DATA PAYMENT DEBIT BANJARMASIN</h3>
			</div>
			<form action="<?php echo $action; ?>" method="post">
				...
				<table class='table table-bordered'>
					...
					<tr>
						<td width='200'>No Debit Nota <?php echo form_error('id_debit_banjarmasin') ?></td>
						<td>
						<select class="form-control" name="id_debit_banjarmasin" id="id_debit_banjarmasin" required>
						<?php 
							foreach($tbl_debit_data as $c):?>

							<option value="<?php echo $c->id_debit_banjarmasin?>" <?php echo ($id_debit_banjarmasin == $c->id_debit_banjarmasin) ? 'selected' : '' ?>>
					...
					<tr>
						<td></td>
						<td>
							<input type="hidden" name="id_payment_banjarmasin" value="<?php echo $id_payment_banjarmasin; ?>" /> 
							<button type="submit" class="btn btn-danger"><i class="fa fa-floppy-o"></i> <?php echo $button ?></button> 
							<a href="<?php echo site_url('tbl_debit_banjarmasin') ?>" class="btn btn-info"><i class="fa fa-sign-out"></i> Kembali</a>
						</td>
					</tr>
